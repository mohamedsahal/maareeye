...
class Auth extends BaseController
{
    ...
    public function login()
    {
        ...
        if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
            ...
            if ($this->ionAuth->login($this->request->getVar('identity'), $this->request->getVar('password'), $remember)) {
                ...
                $response['admin'] = $this->ionAuth->isAdmin();
                $response['message'] = 'admin Logged In successfully';

                if ($this->ionAuth->isVendor()) {

                    $response['error'] = false;
                    $response['csrf_token'] = csrf_token();
                    $response['csrf_hash'] = csrf_hash();
                    $response['vendor'] = $this->ionAuth->isVendor();
                    $response['message'] = 'vendor Logged In successfully';
                }
                if ($this->ionAuth->isDeliveryBoy()) {
                    $val = check_delivery_boy($_POST['identity']);
                    if (!empty($val) && $val['status'] == "1") {
                        $response['error'] = false;
                        $response['csrf_token'] = csrf_token();
                        $response['csrf_hash'] = csrf_hash();
                        $response['delivery_boy'] = $this->ionAuth->isDeliveryBoy();
                        $response['message'] = 'delivery boy Logged In successfully';
                        $this->session->set('business_id', $val['business_id'][0]);
                    } else {
                        $response['error'] = true;
                        $response['csrf_token'] = csrf_token();
                        $response['csrf_hash'] = csrf_hash();
                        $response['message'] = ["you are not Active on this business"];
                ...
                if ($this->ionAuth->isTeamMember()) {
                    ...
                    $response['csrf_token'] = csrf_token();
                    $response['csrf_hash'] = csrf_hash();
                    $response['vendor'] = $this->ionAuth->isTeamMember();
                    $response['message'] = 'Team Member Logged In successfully';
                }
                return $this->response->setJSON($response);
            } else {
    ...
    public function forgot_password()
    {
        ...
        $app_name = (isset($settings['title']) && $settings['title'] != "") ? $settings['title'] : "Maareeye";
        ...
        if (!($this->request->getPost() && $this->validation->withRequest($this->request)->run())) {
            ...
            if ($forgotten) {
                $this->session->setFlashdata('message', $this->ionAuth->messages());
                return redirect()->to('/forgot_password'); //we should display a confirmation page here instead of the login page
            } else {
                $this->session->setFlashdata('message', $this->ionAuth->errors($this->validationListTemplate));
    ...
    public function reset_password($code = null)
    {
        ...
        if ($user) {
            ...
            if (!$this->request->getPost() || $this->validation->withRequest($this->request)->run() === false) {
                ...
                $this->data['code'] = $code;
                $this->data['page'] = VIEWS . 'reset_password';
                // render

                return view('frontend/template', $this->data);
    ...
    public function create_user()
    {
        ...
        // $this->validation->setRule('phone', lang('Auth.create_user_validation_phone_label'), 'trim'); // this filed is not required because identity filed is phone number.
        // $this->validation->setRule('company', lang('Auth.create_user_validation_company_label'), 'trim');  // in some forms this field is not being used so removing it from validation and adding custom logic to handel it 
    ...
    public function edit_user(int $id)
    {
        if (defined('ALLOW_MODIFICATION') && ALLOW_MODIFICATION == 0) {
            ...
            return $this->session->setFlashdata('message', DEMO_MODE_ERROR);
        }
        $this->data['title'] = lang('Auth.edit_user_heading');

        if (!$this->ionAuth->loggedIn() || (!$this->ionAuth->isAdmin() && !$this->ionAuth->isVendor() && !($this->ionAuth->user()->row()->id == $id))) {
        ...
        if (!empty($_POST)) {
            ...
            if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
                $data = [
                    ...
                    'identity' => $this->request->getPost('mobile'),
                ];
                ...
                return redirect()->to('admin/profile')->withCookies();
            }
        }
        ...
        $session = session();
        $session->setFlashdata("message", $this->data['message']);
        ...
        $this->data['mobile'] = [
            ...
            'id' => 'mobile',
            'type' => 'text',
            'value' => set_value('mobile', empty($user->mobile) ? '' : $user->mobile),
        ];
        // $this->data['phone'] = [
        // 	'name'  => 'phone',
        // 	'id'    => 'phone',
