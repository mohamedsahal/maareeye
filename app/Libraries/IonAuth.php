...
use IonAuth\Libraries\IonAuth as BaseIonAuth;
/**
 * Name:    Ion Auth
 *
 * Created:  10.01.2009
...
class IonAuth extends BaseIonAuth
{
    ...
    /**
     * Check to see if the currently logged in user is a vendor.
     *
     * @param integer $id User id
     *
     * @return boolean Whether the user is a vendor
     * @author Ben Edmunds
     */
    public function isVendor(int $id = 0): bool
    {
        $this->ionAuthModel->triggerEvents('is_vendor');
        $vendorGroup = $this->config->vendorGroup;
        return $this->loggedIn() && $this->ionAuthModel->inGroup($vendorGroup, $id);
    }

    public function isTeamMember(int $id = 0): bool
    { 
		// this method is manualy added
        $this->ionAuthModel->triggerEvents('is_team_member');

        $teamMembersGroup = $this->config->teamMembersGroup;

        return $this->loggedIn() && $this->ionAuthModel->inGroup($teamMembersGroup, $id);
    }

    /**
     * Check to see if the currently logged in user is a delivery boy.
     *
     * @param integer $id User id
     *
     * @return boolean Whether the user is a delivery boy
     * @author Ben Edmunds
     */
    public function isDeliveryBoy(int $id = 0): bool
    {
        $this->ionAuthModel->triggerEvents('is_delivery_boy');

        $deliveryBoyGroup = $this->config->deliveryBoyGroup;

        return $this->loggedIn() && $this->ionAuthModel->inGroup($deliveryBoyGroup, $id);
    }
     ...
     * @param integer $id User id
     *
     * @return boolean Whether the user is a customer
     * @author Ben Edmunds
     */
    public function isCustomer(int $id = 0): bool
    {
        $this->ionAuthModel->triggerEvents('is_customer');

        $customerGroup = $this->config->customerGroup;

        return $this->loggedIn() && $this->ionAuthModel->inGroup($customerGroup, $id);
    }
